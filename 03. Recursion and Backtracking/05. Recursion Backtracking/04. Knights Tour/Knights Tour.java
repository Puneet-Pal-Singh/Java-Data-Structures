...
public class Main {
    ...
    /*
        // Smart calls and normal base case 
    ...
    */
    ...
    public static void printKnightsTour(int[][] chess, int r, int c, int nextMove) {
        ...
        // if knight cross length in row and column
        // chess[r][c] > 0 then undo
        if (r < 0 || c < 0 || r >= chess.length || c >= chess[0].length ||
            chess[r][c] > 0) {
            return;
        }
        // else if (nextMove == chess.length * chess[0].length) {
        else if (nextMove == chess.length * chess.length) {
